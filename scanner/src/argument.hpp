    ...
    {
        {
            switch (type) {
                ...
                case underlying_type_t::T_FIXED:   return "dd99::wayland::proto::fixed_point";
                case underlying_type_t::T_STRING:  return "zview";
...
template <> struct std::formatter<format::argument_name_cpp> : std::formatter<std::string_view> {
    inline constexpr auto format(const format::argument_name_cpp & v, format_context & ctx) const
    {
        ...
        bool name_collides = arg.type() == argument_type_t::T_OBJECT || remove_wayland_prefix(arg.interface) == arg.name;
